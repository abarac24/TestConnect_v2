...
class SlaveRtu_MasterTcp(unittest.TestCase):   
    ...
    def __init__(self, ip="127.0.0.1", com="COM5", baudrate=9600):
    ...
    def setUp(self):     
        ...
        self.master = modbus_tcp.TcpMaster(self.ip, 502, 5.0)
        self.master.set_timeout(10.0)
        logger.info("master tcp connected")
        self.server_slave=modbus_rtu.RtuServer(serial.Serial(port=self.com, baudrate=self.baudrate, bytesize=8, parity='N', stopbits=1, timeout=None, xonxoff=False, rtscts=False, dsrdtr=False))
