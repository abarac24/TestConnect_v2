...
class SlaveRtu_MasterTcp(unittest.TestCase):   
    ...
    '''def connection(self, host, com, tcp, baudrate):
        obj=TestConnect.Connect()
        obj.read_testsuite()
        self._host=obj.host
        self._com=obj.scom        
        self._tcpport=obj.tcport
        self._baudrate=obj.srate'''
        
    def setUp(self):     
        """Start testing using RTU over Tcp"""
        
        self.master = modbus_tcp.TcpMaster("192.168.25.12", 502, 5.0)
        #self.master = modbus_tcp.TcpMaster("10.117.65.2", 502, 5.0)
        self.master.set_timeout(10.0)
        logger.info("master tcp connected")
        self.server_slave=modbus_rtu.RtuServer(serial.Serial(port='COM8', baudrate=9600, bytesize=8, parity='N', stopbits=1, timeout=None, xonxoff=False, rtscts=False, dsrdtr=False))
