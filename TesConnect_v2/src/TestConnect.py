...
#from EchoRequestHandler import EchoRequestHandler
import modbus_tk
...
class Connect:
    ...
    def getClientIp(self, host):
        hostIp = ''.join(host)
        dot = hostIp.rfind(".", 0, len(hostIp))
        net = int(hostIp[dot + 1:len(hostIp)]) + 1
        clientIp = hostIp[0:dot + 1] + str(net)
        return clientIp
    ...
    def telnetsession(self, comport, srate, sbytesize, interface, sparity, sstopbits, tcpport, operationmode, telnet):
        host = self.host
        #clientIp=self.getClientIp(host)
        clientIp = socket.gethostbyname(socket.gethostname())
        vgateway = re.findall('([0-9]+)', comport)
        telnet.run_command('set serial ' + comport + ' ' + 'baudrate ' + srate, 0)
        telnet.run_command('set serial ' + comport + ' ' + 'databits ' + sbytesize, 0)
        telnet.run_command('set serial ' + comport + ' ' + 'interface ' + interface, 0)
        telnet.run_command('set serial ' + comport + ' ' + 'parity ' + sparity, 0)
        telnet.run_command('set serial ' + comport + ' ' + 'stopbits ' + sstopbits, 0)

        if operationmode == "rawtcpserver":
            telnet.run_command('set vgateway ' + ''.join(vgateway) + ' ' + operationmode + ' localport' + ' ' + tcpport,
                               0)
            telnet.run_command('set vgateway ' + ''.join(vgateway) + ' mode ' + operationmode, 0)
            telnet.run_command(
                'set vgateway ' + ''.join(vgateway) + ' ' + operationmode + ' datapacking forcetransmit 0', 0)
            telnet.run_command(
                'set vgateway ' + ''.join(vgateway) + ' ' + operationmode + ' datapacking delimiter1enable off', 0)

        elif operationmode == "rawtcpclient":
            telnet.run_command('set vgateway ' + ''.join(vgateway) + ' ' + operationmode+' waitforradio off', 0)
            telnet.run_command(
                'set vgateway ' + ''.join(vgateway) + ' ' + operationmode + ' client1 tcpport' + ' ' + tcpport, 0)
            telnet.run_command('set vgateway ' + ''.join(vgateway) + ' ' + operationmode + ' client1 enable on', 0)
            telnet.run_command('set vgateway ' + ''.join(vgateway) + ' ' + operationmode + ' client1 ip ' + clientIp, 0)
            telnet.run_command('set vgateway ' + ''.join(vgateway) + ' mode ' + operationmode, 0)
            telnet.run_command(
                'set vgateway ' + ''.join(vgateway) + ' ' + operationmode + ' datapacking forcetransmit 0', 0)
            telnet.run_command(
        ...
        elif operationmode == "modbustcpserver":
            telnet.run_command('set vgateway ' + ''.join(vgateway) + ' mode ' + operationmode, 0)
            telnet.run_command('set vgateway ' + ''.join(vgateway) + ' ' + operationmode + ' maxconnections 4', 0)
            telnet.run_command('set vgateway ' + ''.join(vgateway) + ' ' + operationmode + ' localport' + ' ' + tcpport,
                               0)
    ...
    def sendreceive(self):
        if self.pingdevice(self.host) == True:
            iteration = 0
            host_prev = ""
            if self.operationmode == "rawtcpserver":
                ...
                if self.host != self.host_prev:
                    self.telnet = TelnetController.TelnetController(host_name=self.host.strip(), user_name='admin',
                                                                    password='admin', prompt='#')
                    self.telnet.login()
                self.telnetsession(self.comport, self.srate, self.sbytesize, self.interface, self.sparity,
                                   self.sstopbits, self.tcport, self.operationmode, self.telnet)
                self.host_prev = self.host
                s = self.settcp(self.host, int(self.tcport), self.buffer)
                ser = self.setserial(self.sport, int(self.srate), self.stringTObool(self.sxonxoff.lower().capitalize()),
                                     self.stringTObool(self.srtscts.lower().capitalize()),
                                     self.stringTObool(self.sdsrdtr.lower().capitalize()))
                        ...
                        while iteration < int(self.iterations):
                            ...
                            if self.interface == 'loopback':
                                self.logger('Logger.txt',
                                            '   Write data from serial to tcp   ' + '\t' * 20 + '   Write data from tcp to tcp in loop:   ' + '\n')
                            else:
                                self.logger('Logger.txt',
                                            '   Write data from serial to tcp   ' + '\t' * 20 + '   Write data from tcp to serial:   ' + '\n')
                            #self.logger('Logger.txt','='*200+'\n')
                            count_pckt = 0
                            count_pct_rcv = 0
                            count_pct_rcv1 = 0
                            for field in filesend.readlines():
                                ...
                                if ''.join(comp_data).rfind('-', 0, 1) != -1 or ''.join(comp_data).rfind('+', 0,
                                                                                                         1) != -1:
                                    for i in range(0, len(''.join(send_data)), 100):
                                        self.logger('Logger.txt',
                                                    'Send: ' + str(count_pckt) + ' ' + ''.join(send_data.splitlines())[
                                                                                       i:100 + i] + '\n')
                                    for i in range(0, len(''.join(data)), 100):
                                        self.logger('Logger.txt',
                                                    'Received: ' + str(count_pckt) + ' ' + ''.join(data.splitlines())[
                                                                                           i:100 + i] + '\n')
                                ...
                                if ''.join(comp_data).rfind('-', 0, 1) != -1 or ''.join(comp_data).rfind('+', 0,
                                    ...
                                    for i in range(0, len(''.join(send_data)), 100):
                                        self.logger('Logger.txt',
                                                    '\t' * 29 + 'Send: ' + str(count_pckt) + ' ' + ''.join(
                                                        send_data.splitlines())[i:100 + i] + '\n')
                                    for i in range(0, len(''.join(recv_data)), 100):
                                        self.logger('Logger.txt',
                                                    '\t' * 29 + 'Received: ' + str(count_pckt) + ' ' + ''.join(
                                                        recv_data.splitlines())[i:100 + i] + '\n')
                            ...
                            if count_pct_rcv1 != 0:
                                ...
                                ratio_loss1 = 100
                            print 'Send pkt: ' + str(count_pckt) + ' Received pkt: ' + str(
                                count_pct_rcv) + ' Loss packets: ' + str(loss_pkt)
                            print 'Packet loss ratio is: ' + str(round(ratio_loss, 2))
                            #print 'Throughput value: '+str(Throughput.getThroughput(self.host))+' Mbps'
                            self.logger('Logger.txt', '\n')
                            self.logger('Logger.txt', '=' * 200 + '\n')
                            self.logger('Logger.txt',
                                        'Average time to receive ' + self.iterations + ' from serial to tcp ' + str(
                                            round(sum(list_time) / float(len(list_time)),
                                                  2)) + ' seconds' + '\t' * 11 + 'Average time to receive ' + self.iterations + ' from serial to tcp ' + str(
                                            round(sum(list_time) / float(len(list_time)), 2)) + ' seconds' + '\n')
                            self.logger('Logger.txt', 'Send pkt: ' + str(count_pckt) + ' Received pkt: ' + str(
                                count_pct_rcv) + ' Loss packets: ' + str(loss_pkt) + '\t' * 16 + 'Send pkt: ' + str(
                                count_pckt) + ' Received pkt: ' + str(count_pct_rcv1) + ' Loss packets: ' + str(
                                loss_pkt1) + '\n')
                            self.logger('Logger.txt', 'Packet loss ratio is: ' + str(
                                round(ratio_loss, 2)) + '%' + '\t' * 20 + 'Packet loss ratio is: ' + str(
                                round(ratio_loss1, 2)) + '%' + '\n')
            ...
            if self.operationmode == "rawtcpclient":
                ...
                self.logger('Logger.txt', 'Device IP: ' + self.host + '\n')
                self.logger('Logger.txt', 'Device TCP port: ' + self.tcport + '\n')
                self.logger('Logger.txt', 'Raw Tcp Client Ip: ' + socket.gethostbyname(socket.gethostname()) + '\n')
                self.logger('Logger.txt', '=' * 200 + '\n')

                address = (
                socket.gethostbyname(socket.gethostname()), int(self.tcport))  # let the kernel give us a port
                ...
                #####Client tcp
                '''s = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
                s.connect((self.host, int(self.tcport)))'''
                try:
                    ser.open()
                except Exception, e:
                    print "error open serial port: " + str(e)
                ...
                if ser.isOpen():
                    try:
                        ...
                        while iteration < int(self.iterations):
                            ...
                            count_pckt = count_pckt + 1
                            #decomment
                            #ser.write("helllllllllllllllllllllllllllllllllllllllll\n")
                            send_data = "abcdefqewtriopgh#@#^**>?<> "
                            #print send_data
                            ser.write(send_data)
                            #s.send(send_data)

                            #print("write data: helll")
                            time.sleep(10)  #give the serial port sometime to receive the data
                            #numOfLines = 0
                            ...
                            if ''.join(comp_data).rfind('-', 0, 1) != -1 or ''.join(comp_data).rfind('+', 0, 1) != -1:
                                ...
                                for i in range(0, len(''.join(recv_data)), 100):
                                    self.logger('Logger.txt',
                                                'Received: ' + str(count_pckt) + ' ' + ''.join(recv_data.splitlines())[
                                                                                       i:100 + i] + '\n')
                ...
                while iteration < int(self.iterations):
                    ...
                    class SlaveRtu_MasterTcp(unittest.TestCase):
                        ...
                        def setUp(self):
                            ...
                            self.server_slave = modbus_rtu.RtuServer(
                                serial.Serial(port=scom, baudrate=int(srate), bytesize=_bytesize, parity=_parity,
                                              stopbits=_stopbits, timeout=None, xonxoff=_xonxoff, rtscts=_rtscts,
                                              dsrdtr=_dsrdtr))
                        ...
                        def testReadHoldingRegisters(self):
                            ...
                            logger.info(
                                slave_2.set_values("myblock", 500, range(100)))  #write to address 500 the 100 values
                            ...
                            self.assertEqual(tuple(range(100)), result,
                                             'READ_HOLDING_REGISTERS does not contain the expected values')
                        ...
                        def testReadHoldingRegistersOutOfBlock(self):
                            ...
                            self.assertRaises(modbus_tk.modbus.ModbusError, self.master.execute, 1,
                                              modbus_tk.defines.READ_HOLDING_REGISTERS, 300, 10)
                        ...
                        def testWriteSingleRegisters(self):
                            ...
                            result = self.master.execute(1, modbus_tk.defines.WRITE_SINGLE_REGISTER, 10,
                                                         output_value=33)
                        ...
                        def testWriteMultipleRegisters(self):
                            ...
                            result = self.master.execute(1, modbus_tk.defines.WRITE_MULTIPLE_REGISTERS, 10,
                                                         output_value=range(20))
                            self.assertEqual((10, 20), result)
                            self.assertEqual(tuple(range(20)), self.slave1.get_values("hr0-100", 10, 20))

                            result = self.master.execute(1, modbus_tk.defines.WRITE_MULTIPLE_REGISTERS, 1000,
                                                         output_value=range(123))
                        ...
                        def testWriteMultipleRegistersOutOfBlocks(self):
                            ...
                            try:
                                self.master.execute(1, modbus_tk.defines.WRITE_MULTIPLE_REGISTERS, 500,
                                                    output_value=(11, 12))
                            except modbus_tk.modbus.ModbusError, ex:
                                self.assertEqual(ex.get_exception_code(), modbus_tk.defines.ILLEGAL_DATA_ADDRESS)
                                return
                        ...
                        def testWriteMultipleCoils(self):
                            ...
                            result = self.master.execute(1, modbus_tk.defines.WRITE_MULTIPLE_COILS, 0,
                                                         output_value=(1, ))
                            self.assertEqual((0, 1), result)
                            self.assertEqual((1,), self.slave1.get_values("c0-100", 0, 1))

                            result = self.master.execute(1, modbus_tk.defines.WRITE_MULTIPLE_COILS, 10,
                                                         output_value=[1] * 20)
                            self.assertEqual((10, 20), result)
                            self.assertEqual(tuple([1] * 20), self.slave1.get_values("c0-100", 10, 20))

                            result = self.master.execute(1, modbus_tk.defines.WRITE_MULTIPLE_COILS, 1000,
                                                         output_value=[1] * 1968)
                        ...
                        def testWriteMultipleCoilsOutOfBlocks(self):
                            """Check that an error is raised when writing a register out of block"""
                            try:
                                self.master.execute(1, modbus_tk.defines.WRITE_MULTIPLE_COILS, 500,
                                                    output_value=(11, 12))
                            except modbus_tk.modbus.ModbusError, ex:
                                self.assertEqual(ex.get_exception_code(), modbus_tk.defines.ILLEGAL_DATA_ADDRESS)
                                return
                        ...
                        def testBroadcast(self):
                            ...
                            self.master.execute(0, modbus_tk.defines.WRITE_MULTIPLE_REGISTERS, 5000,
                                                output_value=range(20))
                            time.sleep(1)
                            self.master.execute(0, modbus_tk.defines.WRITE_SINGLE_REGISTER, 5030, output_value=12)
                            time.sleep(0.5)
                            self.master.execute(0, modbus_tk.defines.WRITE_MULTIPLE_COILS, 5000,
                                                output_value=tuple([1] * 10))
                            time.sleep(0.5)
            ...
            self.logger('Logger.txt', 'Device with ip ' + self.host + "is not up")
